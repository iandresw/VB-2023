...
Begin {9EB8768B-CDFA-44DF-8F3E-857A8405E1DB} rpAvisoDeCobro 
   ...
   _ExtentX        =   36142
   _ExtentY        =   18468
   SectionData     =   "rpAvisoDeCobro32.dsx":0000
End
...
Public Function DatosGenerales(Identidad As String)
    ...
    If Not RsDatos.EOF Then
        resultado = RsDatos.Fields("ASPE_Seq").Value
        RsDatos.MoveNext
        Do While Not RsDatos.EOF
            resultado = resultado & ", " & RsDatos.Fields("ASPE_Seq").Value
            RsDatos.MoveNext
        Loop
    End If
    RsDatos.Close
    ...
    GranTotal = 0
        If X > rsCont.RecordCount Then Exit Sub
           ...
           Do While Not rsDetalle.EOF
            ...
            Me.txtMontAdeuAnioImpositivo = rsDetalle!aniox
            Me.txtMontAdeuMonto = Format(rsDetalle!Total, "#,###,##0.00")
...
Private Sub ActiveReport_ReportStart()
    ...
    sql = sql & " WHERE (AvPgEnc.AvPgEstado = 1) AND (AvPgEnc.AvPgTipoImpuesto IN (1, 5)) and catastro.codbarrio = '161810001' "
    sql = sql & " GROUP BY Contribuyente.Identidad, Contribuyente.Pnombre, Contribuyente.SNombre, Contribuyente.PApellido, Contribuyente.SApellido"
